...
v0.9.5 adds a context-routing layer for OpenCode models with a 120K window. SQLite still retains the complete ledger and dossier, while each resume pack selects the current method-tree neighborhood, active fields and lineage, relevant gaps, and prioritized pending Observations. Full output/reader contracts enter context only during promotion, assembly, or publishing. Method reuse is compact by default and remains fully recoverable on demand.
...
The default pack ceiling is 52K tokens, with 16K of that ceiling reserved for reasoning and output. Keep the Work Unit in `EXCAVATE` while probing and maintain `context_scope.anchor_execution_node_id`, `anchor_stage_id`, `active_fields`, and `target_fields`. The builder selects nearby execution nodes and only the relevant field slice; omitted counts point to narrow recovery commands.

Switch to `PROMOTE` only when writing canonical ingestion JSON, `ASSEMBLE` when joining reusable facts into the Scenario, and `PUBLISH` for final audit/HTML. Those phases deliberately load the complete output and reader contracts. See `references/CONTEXT_BUDGET_CONTRACT.md` for routing and recovery rules.
