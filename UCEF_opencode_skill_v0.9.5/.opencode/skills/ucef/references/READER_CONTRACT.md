...
6. Which project/module and method invocation implements each stage, and what happens inside each method step?
...
- `TOPOLOGY`
- `EXECUTION_TREE` — every business stage reaches concrete method invocations and internal steps; opaque methods, orphan nodes, and parent cycles are visible failures.
- `ROUTING`
- `FIELD_LINEAGE`
- `PERSISTENCE`
...
The default page begins with business goal, trigger, applicable environment/config, terminal outcome, stores changed, and external systems called. It then shows a synchronized method execution tree, selected-node details, and field data-flow panel. Reused Fragments are expanded inside the relevant node. Source links and raw facts are progressive detail.
