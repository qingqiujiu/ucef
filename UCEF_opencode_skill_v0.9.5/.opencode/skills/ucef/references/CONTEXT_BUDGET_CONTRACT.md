...
The default context pack budget is 52K tokens, with 16K reserved inside that amount for reasoning and output. Pending Observations are selected from a larger candidate pool and ranked by active fields, current focus, next probe, stage, and execution node. Method reuse returns a compact contract by default.
