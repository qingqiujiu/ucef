...
def cmd_doctor(args: argparse.Namespace) -> int:
    ...
    for path in (
        ...
        runtime_root() / "checkpoint.schema.json",
        runtime_root().parent / "references" / "READER_CONTRACT.md",
        runtime_root().parent / "references" / "CHECKPOINT_CONTRACT.md",
        runtime_root().parent / "references" / "EXECUTION_TREE_CONTRACT.md",
    ):
        if not path.exists():
...
def build_parser() -> argparse.ArgumentParser:
    ...
    command.add_argument("--collection", required=True, choices=[
        "dossier", "scenarios", "fragments", "method_definitions", "trace_stages", "execution_nodes",
