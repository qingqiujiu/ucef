# UCEF v0.10.0 Data Model

New analysis writes `scenario_plans`, `business_blocks`, `scenario_overviews`, `analysis_runs`, `analysis_tasks`, and `submission_receipts`. These form the reader-facing three-layer model plus deterministic scheduler state. A `BusinessBlock` is a final reader artifact, not an intermediate delta.

The v0.9.x model below remains available for importing existing workspaces, reusable fragments, detailed method trees, field graphs, checkpoints, and evidence. Those records do not create v0.10 tasks and appear only as a folded technical appendix when direct artifacts exist.

## Legacy compatibility model

v0.9.6 treats `TraceStage` as the business spine, `ImplementationSlice` as cross-method implementation detail, and `CoverageGate` as the programmatic relevance/stop contract. Execution nodes and field lineage remain evidence attached to these layers.
