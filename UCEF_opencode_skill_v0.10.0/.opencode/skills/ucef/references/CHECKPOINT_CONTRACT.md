...
After every semantic probe that returns a chain-changing fact, run `checkpoint` before changing the business question. A semantic probe may use several low-level reads of the same method, route, mapper, client, statement, or configuration question. Also checkpoint before switching projects, before producing a long explanation, and before any likely context compaction, but only when facts or resumable state changed. `max_uncheckpointed_probes` is one.

Do not write a checkpoint merely because a tool call finished, a summary was reworded, or the same result was verified again. The runtime compares the resumable semantic state and returns `no_state_delta` without appending a duplicate. Full audit belongs at promotion/assembly boundaries, not inside the probe loop.
...
Each state-changing Checkpoint records the current focus, visited references, current chain spine, unresolved questions, one next probe, a concise resume summary, and the Observations produced by the just-finished probe. It also retains an `execution_frontier` of unexpanded invocation/step nodes and a `field_frontier` showing where each tracked field currently ends or breaks. The runtime assigns sequence numbers, deduplicates identical Observations, suppresses repeated semantic states, and unions visited references across Checkpoints.
