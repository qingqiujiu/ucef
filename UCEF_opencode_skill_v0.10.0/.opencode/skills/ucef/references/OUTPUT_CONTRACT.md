...
For v0.9.6 layered work, `trace_stages` form the business spine, `implementation_slices` explain cross-method implementation, and `coverage_gates` control what may be expanded. A Slice Work Unit should normally emit one ImplementationSlice, not a rewritten Scenario dossier.

## Implementation slice

Attach every slice to one `stage_id`. Include the actual method path, meaningful internal steps, route/config facts, field effects, boundaries, error/timing behavior, explicit TechnicalBridges, and semantic closure flags. `closure_status=COMPLETE` requires every closure flag to be true; otherwise use `GAP` and name the Gap IDs.

## Coverage gate

Use G0/G1/G2 for work that may auto-expand; G3 is optional and G4 must not auto-expand. A closed gate cites Evidence. A gap-accepted gate cites explicit Gap IDs. Work Units name `closes_gate_ids` instead of claiming general relevance.
