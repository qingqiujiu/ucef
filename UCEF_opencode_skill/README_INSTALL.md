...
- 本次挖掘的清晰业务名称；如果尚不知道要挖哪条链路，明确要求先做有边界的初始项目扫描并建立探索地图；
- 源码范围；
- 用户真正关心的业务问题；
- 已知入口、重点字段或限制条件（如果有）。
...
主 Agent 会查找 `清算通知任务｜全链路分析入口.md`。已有分析从入口链接的调查状态继续；明确新链路创建或更新业务地图、同主题链路封面和状态。若项目陌生或问题宽泛，则先创建/更新`<业务域>｜探索地图.md`，把候选、未知和优先理由分开，再提升最有价值的候选。当前工作目录不会被当成分析位置或唯一源码范围。
...
```text
<业务域>｜探索地图.md                 （陌生项目或宽泛问题时）
<业务域>｜业务地图.md                 （可行动候选或首条真实链路时）
<业务主题>｜全链路分析入口.md         （一条链路的读者封面）
<业务主题>｜调查状态.md
<业务主题>｜业务全链路.md
<业务主题>｜字段生命周期.md
<业务主题>｜关键模块.md
...
用户没有指定目录且不存在可沿用布局时，默认使用 `<业务域>/链路/<当前链路>/` 放置专属内容。初始扫描发现可行动候选或开始第一条真实链路时，就建立 `<业务域>｜业务地图.md`；探索地图只保存候选与未知，业务地图只保存已确认链路、跨链路关系和重大演化。出现真正共享的内容时再建立“公共知识”目录，不预建空结构。目录负责阅读整理；每条链路仍以自己的入口和状态为身份，跨项目链路不按 Git 仓库拆散。

遇到以前分析过的方法、字段或模块时，主 Agent 会先按源项目与完整符号检索旧笔记并核验源码版本。对象自身不随链路变化的部分进入公共知识，当前调用者、参数、命中分支、字段含义和业务结果留在当前链路。拿不准时先不抽象，避免较弱模型错误合并。

## 可选 Obsidian 外观

将 `obsidian/ucef.css` 复制到目标 Vault 的 `.obsidian/snippets/ucef.css`，然后在 Obsidian“外观 → CSS 代码片段”中启用。笔记可使用 `ucef-entry`、`ucef-chain`、`ucef-map`、`ucef-shared` 或 `ucef-state` 作为 `cssclasses`。
...
用户可以移动或重命名正式内容；保留其“归属本次挖掘”Wikilink，并让 Obsidian 更新链接。源码项目迁移时只更新分析入口中的“项目标识 → 实际根目录”映射，不需要重写全部证据。根页面会保留角色、业务域、总体状态和来源快照的轻量 frontmatter；已有人工笔记不要求补齐，调查状态只保存恢复指针，不作为正式内容的副本。
