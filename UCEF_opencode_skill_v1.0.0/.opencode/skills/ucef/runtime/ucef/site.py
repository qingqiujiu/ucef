...
from .experience import AdaptiveExperience
...
class DossierSiteBuilder:
    ...
    def build(self) -> dict[str, Any]:
        ...
        adaptive = AdaptiveExperience(self.store, self.config, self.output).build(scenarios)
        if adaptive.get("adaptive_scenarios"):
            result.update(adaptive)
            result["files"] = sum(path.is_file() for path in self.output.rglob("*.html"))
        return result
