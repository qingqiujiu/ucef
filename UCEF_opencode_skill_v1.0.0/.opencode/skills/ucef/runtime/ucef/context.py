...
from .core import approx_tokens, validate_task_capsule
...
def _compact_implementation_slice(item: dict[str, Any]) -> dict[str, Any]:
    return {
        key: item.get(key)
        for key in (
            "slice_id", "scenario_id", "stage_id", "source_id", "sequence_no", "name",
            "business_purpose", "focus_mode", "method_path", "internal_steps",
            "route_and_config", "field_effects", "boundaries", "error_and_timing",
            "technical_bridges", "semantic_closure", "closure_status", "gap_ids", "status",
        )
        if item.get(key) is not None
    }


def _compact_coverage_gate(item: dict[str, Any]) -> dict[str, Any]:
    return {
        key: item.get(key)
        for key in (
            "gate_id", "scenario_id", "stage_id", "gate_type", "priority", "question",
            "status", "field", "gap_ids",
        )
        if item.get(key) is not None
    }
...
class ContextBuilder:
    ...
    def build(self, scenario: dict[str, Any], work_unit: dict[str, Any]) -> str:
        ...
        total = int(cfg.get("max_total_tokens", 32000))
        reserved = int(cfg.get("reserved_output_tokens", 8000))
        input_budget = max(8000, total - reserved)
        max_observations = int(cfg.get("max_pending_observations", 12))
        ...
        max_nodes = int(cfg.get("max_execution_nodes", 16))
        max_fields = int(cfg.get("max_field_inventory", 8))
        max_lineage_steps = int(cfg.get("max_active_lineage_steps", 24))
        phase = str(work_unit.get("phase") or "EXCAVATE").upper()
        work_type = str(work_unit.get("work_type") or "")
        role = str(work_unit.get("role") or "SLICE").upper()
        scope = work_unit.get("context_scope") or {}
        sections: list[str] = []
        used = 0
        ...
        header = (
            ...
            "persist every state-changing semantic probe as Observations before changing the business question.\n"
        )
        ...
        capsule_errors = validate_task_capsule(work_unit)
        if capsule_errors:
            raise ValueError("Invalid v0.9.6 task capsule: " + "; ".join(capsule_errors))
        ...
        agent_protocol_path = (self.root / "../references/AGENT_PROTOCOL.md").resolve()
        if agent_protocol_path.exists():
            protocol_text = agent_protocol_path.read_text(encoding="utf-8")
            role_contract = _markdown_section(protocol_text, role)
            if role_contract:
                add(f"{role} WORKER CONTRACT", role_contract)
        ...
        stage_anchor = str(scope.get("anchor_stage_id") or "")
        all_slices = dossier.get("implementation_slices") or []
        selected_slices = [
            item for item in all_slices
            if role == "INTEGRATION" or not stage_anchor or str(item.get("stage_id") or "") == stage_anchor
        ][: int(cfg.get("max_implementation_slices", 12))]
        existing["implementation_slices"] = {
            "total": len(all_slices),
            "selected": len(selected_slices),
            "omitted": max(0, len(all_slices) - len(selected_slices)),
            "items": [_compact_implementation_slice(item) for item in selected_slices],
        }
        all_gates = dossier.get("coverage_gates") or []
        requested_gate_ids = {str(value) for value in work_unit.get("closes_gate_ids") or []}
        gate_order = {"G0": 0, "G1": 1, "G2": 2, "G3": 3, "G4": 4}
        selected_gates = sorted(
            all_gates,
            key=lambda item: (
                0 if str(item.get("gate_id")) in requested_gate_ids else 1,
                0 if item.get("status") == "OPEN" else 1,
                gate_order.get(str(item.get("priority")), 9),
            ),
        )[: int(cfg.get("max_coverage_gates", 20))]
        existing["coverage_gates"] = {
            "total": len(all_gates),
            "selected": len(selected_gates),
            "omitted": max(0, len(all_gates) - len(selected_gates)),
            "items": [_compact_coverage_gate(item) for item in selected_gates],
            "dispatch_rule": "Only an OPEN G0/G1/G2 gate or an explicitly requested gate may create automatic follow-up work.",
        }
        if role in {"SLICE", "REPAIR"}:
            ...
            existing["boundary_summary"] = {
                "route_decisions": dossier.get("route_decisions") or [],
                "persistence_effects": dossier.get("persistence_effects") or [],
                "external_interactions": dossier.get("external_interactions") or [],
                "p0_fields": [
                    _compact_field_item(item) for item in dossier.get("field_inventory") or []
