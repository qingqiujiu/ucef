...
-- UCEF 1.0 keeps the legacy dossier tables readable while adding a small,
-- scenario-aware knowledge graph.  Entities are shared across scenarios;
-- memberships and edges carry the scenario-specific perspective.
CREATE TABLE IF NOT EXISTS knowledge_entities (
  ...
  created_at TEXT NOT NULL,
  updated_at TEXT NOT NULL,
  UNIQUE(entity_type, logical_key)
);
CREATE INDEX IF NOT EXISTS idx_knowledge_entities_type
  ON knowledge_entities(entity_type, priority, status, logical_key);
...
CREATE INDEX IF NOT EXISTS idx_knowledge_membership_scenario
  ON knowledge_memberships(scenario_id, sequence_no, entity_id);
...
CREATE TABLE IF NOT EXISTS knowledge_edges (
  ...
  updated_at TEXT NOT NULL,
  FOREIGN KEY(scenario_id) REFERENCES scenarios(scenario_id),
  FOREIGN KEY(source_entity_id) REFERENCES knowledge_entities(entity_id),
  FOREIGN KEY(target_entity_id) REFERENCES knowledge_entities(entity_id)
);
CREATE INDEX IF NOT EXISTS idx_knowledge_edges_source
  ON knowledge_edges(scenario_id, source_entity_id, relation_type);
CREATE INDEX IF NOT EXISTS idx_knowledge_edges_target
  ON knowledge_edges(scenario_id, target_entity_id, relation_type);
...
CREATE INDEX IF NOT EXISTS idx_exploration_sessions_scenario
  ON exploration_sessions(scenario_id, updated_at);

CREATE TABLE IF NOT EXISTS exploration_milestones (
...
CREATE INDEX IF NOT EXISTS idx_exploration_milestones_session
  ON exploration_milestones(session_id, created_at DESC);
'''
...
class FactStore:
    def __init__(self, db_path: str | Path):
        ...
        self.conn.execute("INSERT OR REPLACE INTO meta(key,value) VALUES('schema_version','1.0.0')")
        self.conn.commit()
