...
- 主模型自主提出问题、选择重点、判断深度、组织笔记和选择图形；不再将业务理解压入固定实体 Schema。
- 以 Obsidian Markdown、Wikilink、块 ID、Callout 和可选 Mermaid/Canvas 作为主要阅读体验，移除默认 HTML 生成流程。
- 引入紧凑 `.ucef/state.json` 控制面，明确待探索、正在探索、部分明确、已明确、受阻与暂缓状态，并区分假设、支持、存疑、否定和替代结论。
- 新的轻量工具只提供工作区初始化、状态读取、原子增量更新、断点恢复和链接/证据引用校验；不维护 SQLite 事实图、不计算覆盖率。
- 保持主 Agent 单写入；可选 Probe 只调查一个独立问题并返回证据，不直接修改全局状态或笔记。
- 所有 JSON 与 Markdown 使用 UTF-8，状态写入采用同目录临时文件和原子替换，避免提交边界乱码和半写文件。
