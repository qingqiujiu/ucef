...
class OpenCodeAgentPackagingTests(unittest.TestCase):
    def test_primary_agent_loads_required_skills_and_enables_index_mcp(self):
        ...
        for name, role in (
            ("ucef-skeleton.md", "Skeleton Worker"),
            ("ucef-slice.md", "Slice Worker"),
            ("ucef-integration.md", "Integration Worker"),
        ):
...
class UCEFv09Tests(unittest.TestCase):
    ...
    def test_layered_slice_gate_context_audit_and_site(self):
        ...
        self.assertTrue(any(gap["code"].startswith("NO_SLICE_") for gap in audit["generated_gaps"]))
