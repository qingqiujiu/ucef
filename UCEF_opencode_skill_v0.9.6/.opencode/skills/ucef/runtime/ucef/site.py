...
<div class="chips" style="margin-top:12px"><span class="chip">{len(dossier['trace_stages'])} 业务阶段</span><span class="chip">{len(dossier['implementation_slices'])} 实现切片</span><span class="chip">{len(dossier['execution_nodes'])} 执行树节点</span><span class="chip">{len(dossier['field_inventory'])} 字段清单</span><span class="chip">{len(dossier['field_lineage_steps'])} 字段步骤</span><span class="chip">{len([x for x in dossier['coverage_gates'] if x.get('status')=='OPEN'])} 开放门槛</span><span class="chip">{len([x for x in dossier['gaps'] if x.get('status','OPEN')=='OPEN'])} 缺口</span></div></a>''')
