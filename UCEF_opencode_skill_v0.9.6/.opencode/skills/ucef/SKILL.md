...
Use `TraceStage` as the short business spine. Attach one or more cross-method `ImplementationSlice` records to explain how each relevant step works. Keep source/config/database locations as Evidence. The method execution tree and field graph remain implementation evidence; they do not replace the business story.

Only expand application or framework behavior that changes a decision, P0/necessary P1 field, side effect, error/fallback, transaction/timing, or output. Otherwise record a transparent TechnicalBridge and stop. Automatic work must close a named G0/G1/G2 CoverageGate; symbols without gate impact are deferred or ignored.

Read [references/LAYERED_ANALYSIS_CONTRACT.md](references/LAYERED_ANALYSIS_CONTRACT.md) before creating a v0.9.6 Scenario. When dispatching Workers, also read only the relevant role section in [references/AGENT_PROTOCOL.md](references/AGENT_PROTOCOL.md).
...
Conversation context is a disposable cache, not analysis storage. During excavation, every bounded semantic probe that changes state produces:
...
Run `checkpoint` once after the semantic probe and before switching its business question. Several low-level reads of the same method/config question are one probe. Do not checkpoint `NO_DELTA` results and do not run full audit at checkpoint time. After compaction or a new session, run `resume` and continue only from the latest durable Checkpoint.
...
2. keep the Work Unit in `EXCAVATE`, set `context_scope.anchor_execution_node_id`, `anchor_stage_id`, and `active_fields`, then load only the routed local context;
...
Read [references/WORKFLOW.md](references/WORKFLOW.md) for the execution cycle and [references/CONTEXT_BUDGET_CONTRACT.md](references/CONTEXT_BUDGET_CONTRACT.md) when configuring a 120K model or recovering omitted facts. Switch the Work Unit to `PROMOTE` before creating an ingestion result and read [references/OUTPUT_CONTRACT.md](references/OUTPUT_CONTRACT.md). Use `ASSEMBLE` or `PUBLISH` with [references/READER_CONTRACT.md](references/READER_CONTRACT.md) when auditing or publishing a Scenario. Read [references/DATA_MODEL.md](references/DATA_MODEL.md) only when changing storage or import behavior.
