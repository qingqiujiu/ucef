# UCEF v0.9.6 Workflow

Use three passes: Skeleton creates the business spine and gates; Slice closes one named gate to semantic completion; Integration assembles the reader narrative from compact facts without returning to source code. One semantic probe may use several low-level reads of the same question and writes at most one state-changing Checkpoint.
...
The Work Unit names its objective, entry refs, questions, required outputs, and stop conditions. It also sets `phase=EXCAVATE` plus the current `anchor_execution_node_id`, `anchor_stage_id`, `active_fields`, and `target_fields`. These hints route the context pack to a local tree neighborhood and field slice.
...
Build a resume context and read its latest Checkpoint, prioritized pending Observation frontier, Observation index, and visited-reference union. Chat summaries are not authoritative. If there is an `ACTIVE` Checkpoint, continue from its single `next_probe`. The complete ledger remains in SQLite; use the pack's omitted counts and recovery hints instead of loading the whole dossier.
...
Switch the Work Unit to `phase=PROMOTE` so the full output contract enters the context. Canonical records cite the `observation_ids` they promote. The result contains Evidence plus the relevant Fragment/Scenario records, reusable MethodDefinitions, Scenario execution nodes, field inventory, and field lineage graph. A Scenario-facing discovery must be placed in a business Stage and concrete execution node, or explicitly record why placement is pending.

## 8. Ingest and audit

Use `phase=ASSEMBLE` while connecting records and `phase=PUBLISH` for the final reader audit. Ingestion validates IDs, Evidence, references, and versions. Audit detects broken stage handoffs, opaque methods, orphan/cyclic execution nodes, silent field omission, discontinuous lineage, unresolved routes, incomplete persistence mappings, external parameters without origin/use, and missing terminal outcomes.
