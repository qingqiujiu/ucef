...
This release also contains `.opencode/agents/ucef-java-chain.md`, a selectable primary Agent with all OpenCode permissions allowed. It loads `ucef` and the user's `padb` Skill, enables `index-mcp_*`, and persists at most one state-changing checkpoint per semantic probe. See [OPENCODE_AGENT_SETUP.md](OPENCODE_AGENT_SETUP.md).

v0.9.6 makes the business spine the default reader view and attaches cross-method ImplementationSlices for implementation depth. CoverageGates decide what may auto-expand, while transparent framework plumbing is recorded without following irrelevant internals. The package adds read-only Skeleton, Slice, and Integration subagents with role-specific contracts. Checkpoints occur once per state-changing semantic probe, and the default context input budget is reduced to about 24K.
...
python .opencode/skills/ucef/scripts/ucef.py --workspace D:/ucef/order-analysis method-reuse --source-id order-core --symbol PaymentRouter#route --code-hash HASH
python .opencode/skills/ucef/scripts/ucef.py --workspace D:/ucef/order-analysis method-reuse --source-id order-core --symbol PaymentRouter#route --full
...
3. persist the Observations and current Checkpoint before retrieving another target when the probe changed facts or the resumable frontier;
4. after compaction or a new session, run `resume` and continue from `latest_checkpoint.next_probe`.

`checkpoint` accepts `runs/checkpoint.example.json` format. Identical Observations are deduplicated. Accepted state-changing Checkpoints are append-only; a repeated semantic state is returned as `no_state_delta` and is not written again. Resume packs include a bounded pending frontier, grouped counts, visited references, and explicit omitted counts. Use `memory-query` to retrieve omitted durable facts by subject, kind, source, or Observation ID.
...
## Layered analysis and 120K context policy

The default UCEF budget ceiling is 32K tokens, with 8K reserved for reasoning/output and about 24K available to the compiled input pack. Keep the Work Unit in `EXCAVATE` while probing and maintain its role, named Coverage Gates, anchor stage/node, active fields, acceptance tests, non-goals, allowed operations, and budgets.

Use `ucef-skeleton` to establish the business spine and gates, `ucef-slice` to close one gate with implementation detail, and `ucef-integration` to assemble the global narrative without rereading source code. Workers are read-only and return bounded deltas; the runtime remains the writer.

Switch to `PROMOTE` only when writing canonical ingestion JSON, `ASSEMBLE` when joining reusable facts into the Scenario, and `PUBLISH` for final audit/HTML. See `references/LAYERED_ANALYSIS_CONTRACT.md`, `references/AGENT_PROTOCOL.md`, and `references/CONTEXT_BUDGET_CONTRACT.md`.

`templates/layered_delta.example.json` demonstrates the bounded delta returned by a Slice Worker after the base example has been ingested.
