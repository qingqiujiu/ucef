...
本包包含主调度 Agent `ucef-java-chain`，以及 `ucef-skeleton`、`ucef-slice`、`ucef-integration` 三个短生命周期 Subagent。它们不绑定具体模型，方便在 OpenCode 中选择你当前可用的模型。
...
在控制目录启动 OpenCode，用 Tab（或你的 Agent 切换快捷键）选择 `ucef-java-chain`。主 Agent先建立业务任务和 Coverage Gate，再按角色派发 Subagent；不要直接把“分析整个接口”作为模糊任务下发。首次可以输入：
...
主 Agent frontmatter 使用 `permission: allow`，并显式启用 `index-mcp_*`。Skeleton 和 Slice Subagent 禁止编辑及 Bash，只读源码/配置并返回结构化增量；Integration Subagent还禁用 `index-mcp_*`，只能使用已装配的事实投影。UCEF 运行时负责写入分析工作区和生成 HTML。
