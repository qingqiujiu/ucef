...
v0.9.4 adds field-first excavation and a method execution tree. Reusable method definitions are separated from Scenario-specific invocations; each invocation is decomposed into internal steps with explicit field reads/writes. The Scenario HTML synchronizes a module/method/step tree, selected-node details, and field data-flow paths. Audit rejects opaque methods, unregistered fields, and broken origin-to-sink lineage.
