...
当前开发版采用“强提示词主导推理、Obsidian 承载知识、现有 MCP/Skill 提供事实能力”的架构。面对陌生项目，主模型先以有边界的初始扫描建立探索地图（已观察事实、候选、未知与优先理由），再把有业务证据的候选提升为业务地图与名称明确的链路封面；它们与当前工作目录和源码位置解耦。主模型通过 `index-mcp` 识别并分析一个或多个实际项目，主动委派独立源码问题，在确有需要时通过用户已有的 `padb` 查询数据库。历史方法按稳定语义与当前链路语境复用，来源快照变化只标记受影响结论需复验；交付链路至少维护一张有证据的主图。正式内容可由用户自由调整，调查状态只保存恢复指针，地图只记录重大知识演化，可选 CSS 只改善 Obsidian 外观。UCEF 不再提供自建 Runtime、状态脚本、JSON 控制面、SQLite 事实图或 HTML 生成器。
